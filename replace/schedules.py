"""Time- and token-dependent attention swap schedules.

A `Schedule` maps `t_frac in [0, 1]` (sampling progress) to a swap weight in
[0, 1]: 1.0 keeps source attention (identity), 0.0 lets the target attention
through. A `ScheduleSet` bundles one schedule per token role (preserved,
replaced, context). Vanilla P2P is the special case `Step(tau, 1, 0)` on
preserved + `Constant(0)` on replaced.
"""
import math
