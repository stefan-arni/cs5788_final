...
def shift_noise_map(
    ...
    if feather_sigma > 0:
        ...
        M_tgt_soft = gaussian_blur_mask(M_tgt, sigma=feather_sigma)
    else:
        M_src_soft = M_src.float()
        M_tgt_soft = M_tgt.float()
    ...
    # Zero out both source (object left) and target (shifted content arrives there)
    # to prevent doubling: target previously kept its original eps AND received the
    # shifted source eps on top, which corrupts the noise statistics at that region.
    eps_background = eps_t * (1.0 - M_src_soft) * (1.0 - M_tgt_soft)
    return eps_background + eps_src_shifted
